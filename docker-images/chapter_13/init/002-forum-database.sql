...
INSERT INTO forum.users( username, gecos, email )
SELECT 'Fake author ' || v , 'A non existing author', v || '@foo.bar'
FROM generate_series( 1, 1000 ) v;


INSERT INTO forum.posts( title, content, author, created_on, category )
SELECT 'Article ' || v, md5( v::text ), a.pk, current_date - v, v % 5 + 1
FROM forum.users a, generate_series( 1, 500 ) v;



UPDATE forum.users SET username = 'fluca1978' WHERE pk = 1;
UPDATE forum.posts SET likes = pk % 10;
